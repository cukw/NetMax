...
- если страница открыта по `https://155.212.141.80:8443/`, клиент использует `wss://155.212.141.80:8443/ws`;
...
В UI поле URL сервера скрыто. Для non-web сборок fallback по умолчанию: `wss://155.212.141.80:8443/ws`.

Для релизных native-сборок (Android/iOS/macOS/Windows/Linux) адрес можно зафиксировать при сборке:
```bash
flutter build apk --release --dart-define=NETMAX_SERVER_URL=wss://155.212.141.80:8443/ws
```

Если используется self-signed TLS на телефонах и desktop, можно явно разрешить такие сертификаты в клиенте:
```bash
flutter build apk --release \
  --dart-define=NETMAX_SERVER_URL=wss://155.212.141.80:8443/ws \
  --dart-define=NETMAX_ALLOW_SELF_SIGNED_CERT=true
```
...
- если не задан, используется `wss://155.212.141.80:8443/ws`;
- для self-signed можно задать repo variable `NETMAX_ALLOW_SELF_SIGNED_CERT=true` (по умолчанию `false`).
...
Что делает конфиг:
- редирект `80 -> 8443`;
- `wss://155.212.141.80:8443/ws` проксируется на backend `127.0.0.1:8080/ws`;
...
Backend лучше запускать локально на сервере и не публиковать порт 8080 наружу (доступ только через Nginx/8443).
...
- Для Android в приложении включено доверие `user` CA (`android/app/src/main/res/xml/network_security_config.xml`), поэтому после установки вашего CA-сертификата в систему подключение по TLS проходит без отключения проверки.
- Параметр `NETMAX_ALLOW_SELF_SIGNED_CERT=true` отключает строгую проверку цепочки сертификатов в native-клиентах. Используйте только в закрытом контуре/тесте.
